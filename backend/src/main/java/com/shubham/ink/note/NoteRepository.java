...
public interface NoteRepository extends JpaRepository<Note, UUID> {
    ...
    @Query("SELECT n FROM Note n WHERE n.user.id = :userId AND n.archived = false AND (LOWER(n.title) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(n.content) LIKE LOWER(CONCAT('%', :query, '%'))) ORDER BY n.pinned DESC, n.updatedAt DESC")
    Page<Note> searchActiveNotes(@Param("userId") UUID userId, @Param("query") String query, Pageable pageable);
